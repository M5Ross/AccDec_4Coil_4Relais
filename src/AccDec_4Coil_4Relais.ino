...
//   With sincere thanks
      ...
      /*********************************************
       *                                           *
       *  modified by M5Ross => AccDec4Coil4Relè   *
       *                                           *
       *********************************************/
...
// Make inactive selected turnaut for defined time
...
byte FApins [] = {3,5,7,9};
byte FBpins [] = {4,6,8,10};
byte Rpins [] = {19,18,17,16};
byte input [] = {12,13,14,15};
...
boolean pressed(byte in) {
  if(millis()-lastPressTime[in] < tempo) {
    return false;
  }
  lastPressTime[in] = millis();

  byte pos = digitalRead(input[in]);
  
  if(pos == LOW && last_pos == LOW) {
    last_pos = LOW;
    // il pulsante è già stato premuto e lo è ancora quindi non faccio niente
    return false;
  }
  if(pos == LOW && last_pos == HIGH) {
    last_pos = LOW;
    // il pulsante è stato premuto quindi agisco
    return true;
  }
  if(pos == HIGH && last_pos == LOW) {
    last_pos = HIGH;
    tempo = 500;
    // il pulsante è stato rilasciato quidi aspetto che venga ripremuto e non faccio nulla ora
    return false;
  }
  if(pos == HIGH && last_pos == HIGH) {
    last_pos = HIGH;
    tempo = 100;
    // il pulsante non è premuto non faccio niente
    return false;
  }
...
extern void notifyDccAccState( uint16_t Addr, uint16_t BoardAddr, uint8_t OutputAddr, uint8_t State) {
  ...
  if ( Addr >= Current_Decoder_Addr && Addr < Current_Decoder_Addr+4) { //Controls Accessory_Address+3
#ifdef DEBUG
	 Serial.print("Addr = ");
	 Serial.println(Addr);
